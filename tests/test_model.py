...
# For test
build_compact_model = partial(WaveNet, layers=6, stacks=2,
...
build_student_model = partial(StudentWaveNet,
                              residual_channels=32,
                              gate_channels=32,
                              #   skip_out_channels=32,
                              out_channels=2,
...
def test_wavenet():
    model = build_student_model().cuda()
    print(model)
    x = Variable(torch.zeros(16, 1, 1000)).cuda()
    y = model(x)
    print(y.size())
...
def test_mixture_wavenet():
    ...
    y_online2 = model.incremental_forward(test_inputs=None, c=c, T=T, tqdm=tqdm)
